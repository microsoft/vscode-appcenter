 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
function _update(orgName, userEmail, userRole, options, callback) {
   /* jshint validthis: true */
  let client = this.client;
  if(!callback && typeof options === 'function') {
    callback = options;
  ...
  try {
    ...
    if (userRole === null || userRole === undefined) {
      throw new Error('userRole cannot be null or undefined.');
    }
  } catch (error) {
    return callback(error);
  }
...
class OrgInvitations {
   ...
   * @reject {Error} - The error object.
   */
  updateWithHttpOperationResponse(orgName, userEmail, userRole, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._update(orgName, userEmail, userRole, options, (err, result, request, response) => {
        let httpOperationResponse = new msRest.HttpOperationResponse(request, response);
        httpOperationResponse.body = result;
        if (err) { reject(err); }
        else { resolve(httpOperationResponse); }
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  update(orgName, userEmail, userRole, options, optionalCallback) {
    let client = this.client;
    let self = this;
    if (!optionalCallback && typeof options === 'function') {
      optionalCallback = options;
    ...
    if (!optionalCallback) {
      ...
      return self._update(orgName, userEmail, userRole, options, optionalCallback);
    }
  }
