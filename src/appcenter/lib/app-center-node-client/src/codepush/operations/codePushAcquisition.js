...
function _updateDeployStatus(releaseMetadata, options, callback) {
  ...
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'v0.1/public/hotfix/report_status/deploy';
...
function _updateDownloadStatus(releaseMetadata, options, callback) {
  ...
  // Construct URL
  let baseUrl = this.client.baseUri;
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'v0.1/public/hotfix/report_status/download';
 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
function _getAcquisitionStatus(options, callback) {
   /* jshint validthis: true */
  let client = this.client;
  if(!callback && typeof options === 'function') {
    callback = options;
  ...
  // Construct URL
  let baseUrl = this.client.baseUri;
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'v0.1/public/hotfix/status';
    ...
    if (statusCode === 200) {
      ...
      try {
        ...
        if (parsedResponse !== null && parsedResponse !== undefined) {
          let resultMapper = new client.models['AcquisitionStatusSuccessResponse']().mapper();
...
function _updateCheck(appVersion, deploymentKey, options, callback) {
  ...
  // Construct URL
  let baseUrl = this.client.baseUri;
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'v0.1/public/hotfix/update_check';
  let queryParameters = [];
...
class CodePushAcquisition {
  ...
  constructor(client) {
    ...
    this._getAcquisitionStatus = _getAcquisitionStatus;
   ...
   * @reject {Error} - The error object.
   */
  getAcquisitionStatusWithHttpOperationResponse(options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._getAcquisitionStatus(options, (err, result, request, response) => {
        let httpOperationResponse = new msRest.HttpOperationResponse(request, response);
        httpOperationResponse.body = result;
        if (err) { reject(err); }
        else { resolve(httpOperationResponse); }
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  getAcquisitionStatus(options, optionalCallback) {
    let client = this.client;
    let self = this;
    if (!optionalCallback && typeof options === 'function') {
      optionalCallback = options;
    ...
    if (!optionalCallback) {
      ...
      return self._getAcquisitionStatus(options, optionalCallback);
    }
  }
