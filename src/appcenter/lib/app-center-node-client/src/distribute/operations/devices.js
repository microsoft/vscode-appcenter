 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
function _getReleaseUpdateDevicesStatus(appName, ownerName, releaseId, resignId, options, callback) {
   /* jshint validthis: true */
  let client = this.client;
  if(!callback && typeof options === 'function') {
    callback = options;
...
class Devices {
   ...
   * @reject {Error} - The error object.
   */
  getReleaseUpdateDevicesStatusWithHttpOperationResponse(appName, ownerName, releaseId, resignId, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._getReleaseUpdateDevicesStatus(appName, ownerName, releaseId, resignId, options, (err, result, request, response) => {
        let httpOperationResponse = new msRest.HttpOperationResponse(request, response);
        httpOperationResponse.body = result;
        if (err) { reject(err); }
        else { resolve(httpOperationResponse); }
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  getReleaseUpdateDevicesStatus(appName, ownerName, releaseId, resignId, options, optionalCallback) {
    let client = this.client;
    let self = this;
    if (!optionalCallback && typeof options === 'function') {
      optionalCallback = options;
    ...
    if (!optionalCallback) {
      ...
      return self._getReleaseUpdateDevicesStatus(appName, ownerName, releaseId, resignId, options, optionalCallback);
    }
  }
